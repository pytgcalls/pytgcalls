...
class UpdateSources(Scaffold):
    async def _update_sources(
        ...
        for x in participants:
            if (
                x.video_info is not None
                and x.user_id not in self._call_sources[chat_id].camera
            ):
                self._call_sources[chat_id].camera[x.user_id] = (
            ...
            if (
                x.presentation_info is not None
                and x.user_id not in self._call_sources[chat_id].presentation
            ):
                self._call_sources[chat_id].presentation[x.user_id] = (
