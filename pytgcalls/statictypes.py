...
def statictypes(func):
    ...
    def is_instance(obj, typ):
        ...
        if origin is Union:
            ...
            if obj:
                return all(
                    is_instance(x, typ.__args__[0]) and is_instance(obj[x], typ.__args__[1]) for x in obj
                )
    ...
    def type_to_string(t):
        ...
        if origin is Union:
            ...
            return 'Union[' + ', '.join(type_to_string(tt) for tt in args) + ']'
        elif origin in {list, dict, set, tuple}:
            return f'{t.__origin__.__name__.capitalize()}[{", ".join(type_to_string(tt) for tt in t.__args__)}]'
        return t.__name__
