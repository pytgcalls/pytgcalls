...
class HydrogramClient(BridgedClient):
        ...
        async def on_update(_, update, __, data2):
            if isinstance(update, UpdateGroupCallParticipants):
                ...
                for participant in participants:
                    ...
                    if result is not None and 'PARTICIPANTS_HANDLER' in self.HANDLERS_LIST:
            ...
            if isinstance(update, UpdateChannel):
                ...
                if len(data2) > 0 and isinstance(data2[update.channel_id], ChannelForbidden):
                    self._cache.drop_cache(chat_id)
            ...
            if isinstance(update, (UpdateNewChannelMessage, UpdateNewMessage)):
                if isinstance(update.message, MessageService):
                    if isinstance(update.message.action, MessageActionInviteToGroupCall) and 'INVITE_HANDLER' in self.HANDLERS_LIST:
            ...
            if isinstance(data2, Dict):
                for group_id in data2:
                    if isinstance(update, (UpdateNewChannelMessage, UpdateNewMessage)):
                        if isinstance(update.message, MessageService):
                            if isinstance(data2[group_id], (Channel, Chat)) and data2[group_id].left:
    ...
    def on_closed_voice_chat(self) -> Callable:
        return self._register_handler('CLOSED_HANDLER')

    def on_kicked(self) -> Callable:
        return self._register_handler('KICK_HANDLER')

    def on_receive_invite(self) -> Callable:
        return self._register_handler('INVITE_HANDLER')

    def on_left_group(self) -> Callable:
        return self._register_handler('LEFT_HANDLER')

    def on_participants_change(self) -> Callable:
        return self._register_handler('PARTICIPANTS_HANDLER')
    ...
    async def get_call(self, chat_id: int) -> Optional[InputGroupCall]:
        ...
        if isinstance(chat, InputPeerChannel):
            input_call = (await self._app.invoke(GetFullChannel(channel=InputChannel(channel_id=chat.channel_id, access_hash=chat.access_hash)))).full_chat.call
        else:
            input_call = (await self._app.invoke(GetFullChat(chat_id=chat.chat_id))).full_chat.call

        if input_call is not None:
            call: GroupCall = (await self._app.invoke(GetGroupCall(call=input_call, limit=-1))).call
    ...
    async def get_participants(self, input_call: InputGroupCall):
        return [
            ...
            } for participant in (await self._app.invoke(GetGroupParticipants(call=input_call, ids=[], sources=[], offset='', limit=500))).participants
        ]
    ...
    async def leave_group_call(self, chat_id: int):
        chat_call = await self._cache.get_full_chat(chat_id)
        if chat_call is not None:
            await self._app.invoke(LeaveGroupCall(call=chat_call, source=0))

    async def change_volume(self, chat_id: int, volume: int, participant: InputPeer):
        chat_call = await self._cache.get_full_chat(chat_id)
        if chat_call is not None:
            await self._app.invoke(EditGroupCallParticipant(call=chat_call, participant=participant, muted=False, volume=volume * 100))
