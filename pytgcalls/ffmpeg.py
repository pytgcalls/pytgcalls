...
from .exceptions import InvalidVideoProportion, NoAudioSourceFound, NoVideoSourceFound
...
async def check_stream(
    ...
    try:
        ffprobe = await asyncio.create_subprocess_exec(
            *tuple(await cleanup_commands(build_command(
                'ffprobe', ffmpeg_parameters, path, stream_parameters, before_commands, headers,
            ))),
        ...
        if stream_parameters and isinstance(stream_parameters, VideoParameters):
            if not have_video:
        ...
        if stream_parameters and isinstance(stream_parameters, AudioParameters) and not have_audio:
...
async def cleanup_commands(commands: List[str]) -> List[str]:
    try:
        ...
        result = ''
        try:
            stdout, _ = await asyncio.wait_for(proc_res.communicate(), timeout=30)
            result = stdout.decode('utf-8')
        except (subprocess.TimeoutExpired, JSONDecodeError):
            pass

        supported = re.findall(r'(-.*?)\s+', result)
        new_commands = [v for v in commands if v[0] != '-' or v in supported]
...
def build_command(
    ...
    ffmpeg_command = [name] + command['start']
    ...
    if headers:
        ffmpeg_command.extend(['-headers', _build_headers_string(headers)])
...
def _get_stream_params(command: str):
    ...
    command_args = {arg: [] for arg in arg_names}
...
def _build_headers_string(headers: Dict[str, str]) -> str:
    return '"{}"'.format(''.join([f'{key}: {value}\r\n' for key, value in headers.items()]))

def _build_ffmpeg_options(stream_parameters: VideoParameters) -> List[str]:
    ...
    if isinstance(stream_parameters, VideoParameters):
        options.extend(['rawvideo', '-r', str(stream_parameters.frame_rate), '-pix_fmt', 'yuv420p',
                        '-vf', f'scale={stream_parameters.width}:{stream_parameters.height}'])
    else:
        options.extend(['s16le', '-ac', str(stream_parameters.channels), '-ar', str(stream_parameters.bitrate)])
