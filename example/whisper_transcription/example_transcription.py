...
from pytgcalls.events import StreamFrames, ChatUpdate
from pytgcalls.types import MediaStream
from pytgcalls.types.calls import GroupCallParticipant
from pytgcalls.types.stream import Device, Direction
...
CHAT_ID = int(os.getenv("CHAT_ID", "-1001234567890")) # Replace with target chat ID
...
ssrc_to_stream_metadata = {} # SSRC -> { 'media_type': 'audio'/'video', 'start_time_ms': ..., 'last_frame_timestamp_ms': ..., 'video_width': ..., 'video_height': ... }
...
async def record_telegram_call(chat_id_to_record: int):
    ...
    if SESSION_STRING:
        telethon_client_global = TelegramClient(StringSession(SESSION_STRING), API_ID, API_HASH)
    else:
        telethon_client_global = TelegramClient("anon_recorder_session", API_ID, API_HASH)

    await telethon_client_global.connect()
    if not await telethon_client_global.is_user_authorized():
        logger.error("User is not authorized. Please run a script to log in and save session.")
    ...
    archive_dir_current_call = os.path.join(ROOT_ARCHIVE_DIR, f"call_{chat_id_to_record}_{call_start_time_str}")
    ...
    try:
        logger.info(f"Attempting to join call in chat {chat_id_to_record} as a listener.")
        ...
        while keep_running:
            ...
            logger.debug(f"Recording in progress... {active_streams} active streams. Checking for stop signal.")
            await asyncio.sleep(5) # Check every 5 seconds for participants and stop signal
        ...
        call_metadata["streams"] = list(ssrc_to_stream_metadata.values()) # Store copies of stream metadata
        metadata_file_path = os.path.join(archive_dir_current_call, "metadata.json")
        try:
...
async def update_participant_and_ssrc_maps(current_chat_id: int):
    ...
    if not pytg_app_global or not pytg_app_global.is_running:
        logger.warning("PyTgCalls not running, cannot update participant maps.")
        return

    try:
        call_info = (await pytg_app_global.calls).get(current_chat_id)
        if not call_info or not call_info.participants:
            logger.debug(f"No call info or participants found for chat {current_chat_id} to update maps.")
            return

        current_participants = call_info.participants
        logger.debug(f"Found {len(current_participants)} participants in call {current_chat_id}.")
        ...
        for p_info in current_participants: # p_info is GroupCallParticipant
            ...
            if not participant_id: # Should not happen if user_id is a direct attribute
                logger.warning(f"Participant with no ID found (should be impossible): {p_info}. Skipping.")
                continue
            ...
            if p_info.source is not None: # p_info.source is the audio SSRC
                ...
                new_participant_info_map[ssrc_audio] = {"id": participant_id, "name": participant_name, "type": "audio"}
            ...
            if p_info.video_info and p_info.video_info.sources is not None and len(p_info.video_info.sources) > 0:
                for ssrc_group in p_info.video_info.sources: # ssrc_group is SsrcGroup from ntgcalls
                    if hasattr(ssrc_group, 'ssrc'):
                        ...
                        new_participant_info_map[video_ssrc] = {"id": participant_id, "name": participant_name, "type": "video"}
                        new_participant_to_ssrcs_map[participant_id].add(video_ssrc)
                        new_ssrc_to_participant_id_map[video_ssrc] = participant_id
                        logger.info(f"Found video SSRC {video_ssrc} for participant {participant_id} ({participant_name})")
                    else:
                        logger.warning(f"Video SsrcGroup for participant {participant_id} ({participant_name}) does not have 'ssrc' attribute. SsrcGroup: {ssrc_group}")
            else:
                logger.debug(f"No video sources or video_info for participant {participant_id} ({participant_name}). Video_info: {p_info.video_info}")
            ...
            if p_info.presentation_info and p_info.presentation_info.sources is not None and len(p_info.presentation_info.sources) > 0:
                for ssrc_group in p_info.presentation_info.sources: # ssrc_group is SsrcGroup from ntgcalls
                    if hasattr(ssrc_group, 'ssrc'):
                        ...
                        new_participant_info_map[screen_ssrc] = {"id": participant_id, "name": participant_name, "type": "screen"}
                        new_participant_to_ssrcs_map[participant_id].add(screen_ssrc)
                        new_ssrc_to_participant_id_map[screen_ssrc] = participant_id
                        logger.info(f"Found screen share SSRC {screen_ssrc} for participant {participant_id} ({participant_name})")
                    else:
                        logger.warning(f"Screen share SsrcGroup for participant {participant_id} ({participant_name}) does not have 'ssrc' attribute. SsrcGroup: {ssrc_group}")
            else:
                logger.debug(f"No presentation sources or presentation_info for participant {participant_id} ({participant_name}). Presentation_info: {p_info.presentation_info}")
        ...
        logger.info(f"Participant maps updated. {len(participant_info_map)} SSRCs mapped.")
        logger.debug(f"SSRC to PID map: {ssrc_to_participant_id_map}")
        logger.debug(f"PID to SSRCs map: {participant_to_ssrcs_map}")

    except Exception as e:
        logger.error(f"Error updating participant and SSRC maps: {e}", exc_info=True)
...
async def handle_stream_frames_event(client: PyTgCalls, event: StreamFrames):
    ...
    for frame_obj in event.frames: # frame_obj is Frame
        ...
        media_type = "unknown"
        file_extension = "raw" # Default for raw Opus audio (event.device == MICROPHONE)
        ...
        if participant_details and participant_details.get("type") == "unknown" and media_type != "unknown":
            ...
            logger.info(f"Updated media type for SSRC {ssrc} to {media_type} based on event.device")


        if ssrc not in ssrc_to_file_map:
            if not archive_dir_current_call:
                logger.error(f"Archive directory not set, cannot create file for SSRC {ssrc}")
            ...
            try:
                ...
                logger.info(f"Opened new file for SSRC {ssrc} ({media_type}): {filepath}")
                ...
                logger.error(f"Error opening file for SSRC {ssrc}: {filepath}. Error: {e}")
                if ssrc in ssrc_to_file_map: del ssrc_to_file_map[ssrc]
                if ssrc in ssrc_to_stream_metadata: del ssrc_to_stream_metadata[ssrc]
                continue # Skip this frame_obj
        ...
        if ssrc in ssrc_to_file_map:
            try:
                ...
                if frame_data:
                    ...
                    # Update metadata
                    current_capture_time_ms = frame_obj.info.capture_time if frame_obj.info and frame_obj.info.capture_time else int(time.time() * 1000)
                    ...
                    logger.warning(f"Received empty frame data for SSRC {ssrc}")
                ...
                logger.error(f"Unexpected error processing frame for SSRC {ssrc}: {e}", exc_info=True)
...
async def main():
    global pytg_app_global # To ensure it's accessible in signal handler for cleanup if needed
    ...
    if not CHAT_ID:
        logger.error("CHAT_ID is not set. Please set the environment variable or update the script.")
        return
...
if __name__ == "__main__":
    ...
    try:
        ...
        if pytg_app_global and pytg_app_global.is_running:
            logger.warning("PyTgCalls still running in final cleanup, attempting to stop.")
