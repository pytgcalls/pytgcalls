...
class MemoryTest:
    ...
    async def test_cache_operations(self, call_py) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_increase': after_cache_memory['current_mb'] - initial_memory['current_mb'],
            'cleanup_reduction': after_cache_memory['current_mb'] - after_cleanup_memory['current_mb'],
        }
    ...
    async def test_handler_operations(self, call_py) -> Dict[str, Any]:
        ...
        for i in range(100):
            def handler_func(update):
                return f"Handler {i} processed: {update}"
            
            call_py.add_handler(handler_func)
        ...
        return {
            ...
            'handler_increase': after_handlers_memory['current_mb'] - initial_memory['current_mb'],
            'clear_reduction': after_handlers_memory['current_mb'] - after_clear_memory['current_mb'],
        }
    ...
    async def test_chat_lock_operations(self, call_py) -> Dict[str, Any]:
        ...
        return {
            ...
            'after_clear_memory': after_clear_memory,
            'lock_increase': after_locks_memory['current_mb'] - initial_memory['current_mb'],
            'clear_reduction': after_locks_memory['current_mb'] - after_clear_memory['current_mb'],
        }
    ...
    async def test_comprehensive_cleanup(self, call_py) -> Dict[str, Any]:
        ...
        return {
            ...
            'operations_increase': after_operations_memory['current_mb'] - initial_memory['current_mb'],
            'shutdown_reduction': after_operations_memory['current_mb'] - after_shutdown_memory['current_mb'],
        }
    ...
    def print_test_summary(self):
        ...
        for test_name, results in self.test_results.items():
            ...
            if 'cache_increase' in results:
                logger.info(f"Cache Increase: {results['cache_increase']:.2f}MB")
                logger.info(f"Cleanup Reduction: {results['cleanup_reduction']:.2f}MB")
            
            if 'handler_increase' in results:
                logger.info(f"Handler Increase: {results['handler_increase']:.2f}MB")
                logger.info(f"Clear Reduction: {results['clear_reduction']:.2f}MB")
            ...
            if 'operations_increase' in results:
                logger.info(f"Operations Increase: {results['operations_increase']:.2f}MB")
                logger.info(f"Shutdown Reduction: {results['shutdown_reduction']:.2f}MB")
        
        # Final memory stats
        final_memory = self.get_memory_usage()
        logger.info(f"\nFINAL MEMORY USAGE:")
        logger.info(f"Current: {final_memory['current_mb']:.2f}MB")
        logger.info(f"Peak: {final_memory['peak_mb']:.2f}MB")
...
async def main():
    ...
    try:
        ...
        # For demonstration, we'll create a mock object with the required attributes
        ...
        # Run tests
        test.test_results['cache_test'] = await test.test_cache_operations(call_py)
        test.test_results['handler_test'] = await test.test_handler_operations(call_py)
        test.test_results['chat_lock_test'] = await test.test_chat_lock_operations(call_py)
        
        # Create new instance for comprehensive test
        call_py2 = MockPyTgCalls()
        test.test_results['comprehensive_test'] = await test.test_comprehensive_cleanup(call_py2)
