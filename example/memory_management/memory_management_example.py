    ...
    if stats:
        ...
        if 'user_peer_cache' in stats:
            ...
            response += (
                ...
                f"• Cache size: {cache_stats['before_size']} → {cache_stats['after_size']}\n"
            )
        
        if 'client_cache' in stats:
            cache_stats = stats['client_cache']
            response += f"• Client Cache: {cache_stats['cleaned']} items cleaned\n"
        
        if 'garbage_collection' in stats:
            gc_stats = stats['garbage_collection']
            response += f"• Garbage Collection: {gc_stats['collected_objects']} objects collected\n"
    else:
...
async def stress_test_command(client: Client, message: Message):
    ...
    response = (
        ...
        f"• Created 100 temporary cache entries\n"
        f"• Performed cleanup\n"
    )
